/*
 * There is a lot of static data here. We should have an OO version.
...
gl_putc(int c)
{
    ...
    if ( !gl_passwd || !isgraph(c))
    {
       ...
       if(write(1, &ch, 1)) {} /*G.Barrand : g++-4.4.1 : if() */
    }
...
#else
    if(write(1, buf, len)) {} /*G.Barrand : g++-4.4.1 : if() */
#endif
}
...
#else
    if(write(2, buf, len)) {} /*G.Barrand : g++-4.4.1 : if() */
#endif
    exit(1);
}
...
int Gl_is_there_stdin_input(int* aIs) {
  ...
  if(!PeekConsoleInput(hConsoleInput,&pirBuffer,1,&cRead)) {
    *aIs = 0;
    return 0;
  }    
  ...
  // Should correspond to what is done in Getline.c/pause function for WIN32 :

  KeyEvent = (KEY_EVENT_RECORD*)&(pirBuffer.Event);
  if((pirBuffer.EventType==KEY_EVENT)&&(KeyEvent->bKeyDown==TRUE)){
    *aIs = 1;
  } else {
    ReadConsoleInput(hConsoleInput,&pirBuffer,1,&cRead);
    *aIs = 0;
  }
